...
const Card = (property: PropertyProps) => {
    return (
        ...
        <Link href={`/property/${property.name}`}>
            ...
            <div>
                <h3 className="font-bold text-[22px]">{property.name}</h3>
                <h4 className="text-gray-400 text-[17px]">{property.address.state}, {property.address.city}, {property.address.country}</h4>
            </div>
            ...
            <div className="flex items-end">
                <p className="text-[22px] font-semibold">${property.price}</p>
